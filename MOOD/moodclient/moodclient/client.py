...
class Game(cmd.Cmd):
    ...
    def do_addmon(self, args):
        ...
        if len(args := shlex.split(args)) == 8 and args[0] in list_cows:
            ...
            self._socket.send((msg.strip().lower() + "\n").encode())
...
def get_reponse(_socket, cmdline):
    while True:
...
def start(_socket):
    ...
    gm = threading.Thread(target=get_reponse, args=(_socket, cmdline))
    gm.start()
